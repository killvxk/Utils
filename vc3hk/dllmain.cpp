	...
	if (!POINTERCHK(pBED)) return;
		...
		if (iWeaponID != -1) {
				...
				)&&iWeaponID != cl_SoldierWeapon)
			{
				...
				if (pBED->m_startDamage > 79.0f&&pBED->m_startDamage<95.1f)pBED->m_startDamage = 191.0f;
			...
			//	printf_s("damagestart: %f,end: %f\n", pBED->m_startDamage, pBED->m_endDamage);
				if (pBED->m_startDamage >= pBED->m_endDamage) {
					pBED->m_endDamage = pBED->m_startDamage;

				}
				//	if (ori_startdamage > 0)pBED->m_startDamage = ori_startdamage*1.1f;
					//if (ori_enddamage > 0)pBED->m_endDamage = ori_enddamage*1.1f;
		...
		if (!POINTERCHK(pBED)) {
			...
			if (flBulletdamage > 2.00 && flBulletspeed > 40.0f)
			{
				if (bInstantKill && flBulletdamage != 999.0f)
				{
