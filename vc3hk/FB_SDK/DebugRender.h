...
/*
	...
	public:
		...
		*/
		...
		{
			DWORD DBGRENDERER2=0x004B2EA0;
			typedef DebugRenderer2* ( __stdcall* tDebugRenderer2 )();
			static tDebugRenderer2 oDebugRenderer2 = ( tDebugRenderer2 )DBGRENDERER2;
		...
		*/
		...
		void DrawText(float x, float y, float scale, Color32 color, char* text, ...) {
			...
			va_start(va, text);
			_vsnprintf_s(szBuffer, sizeof(szBuffer), text, va);
			va_end(va);

			typedef void(__thiscall* tDrawText)(DebugRenderer2*, int, int, char*, Color32, float);
			tDrawText oDrawText = ( tDrawText )0x004B7610;
		...
		void DrawLine(float x1, float y1, float x2, float y2, Color32 color) {
			...
			tDrawLine oDrawLine = ( tDrawLine )0x004B9980;
