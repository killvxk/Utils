...
int   SolveP4De(double *x, double b, double c, double d)	// solve equation x^4 + b*x^2 + c*x + d
{
	...
	{				
		...
		{
			x[0] = -sz3/2;					
			x[1] = ( sz1 -sz2)/2;		// x[0]眎*x[1]
			x[2] =  sz3/2;
			x[3] = (-sz1 -sz2)/2;		// x[2]眎*x[3]
			return 0;
		}
		// now: c<0 , sign = +1
		x[0] =   sz3/2;
...
// return 2: 2 real roots x[0], x[1] and complex x[2]眎*x[3], 
// return 0: two pair of complex roots: x[0]眎*x[1],  x[2]眎*x[3], 
